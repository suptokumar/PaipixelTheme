...
<form action="" id="search_user">
	...
	<select id="class" onchange="search_d52(1)" style="padding: 6px 6px; float: right;" name="class">
		<option value="">Teacher Student Both</option>
		<option value="1">Only Students</option>
		<option value="2">Only Teachers</option>
	</select>
...
function search_d52(page){
	...
	var key = $("#ms25").val();
	var data = "class="+classs+"&&key="+key+"&&page="+page;
	$.ajax({
		url: '<?php get_domain("/") ?>ajax/rating/contributions.php',
		type: 'GET',
		data: data,
	})
	.done(function(datas) {
