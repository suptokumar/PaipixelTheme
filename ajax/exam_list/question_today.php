...
include '../extra/db.extra.php';
	$id = date("Y-m-d",$_GET['id']);
$sql = "SELECT * FROM todays_model WHERE date='$id'";
...
<style>
	...
	#squestion<?php echo $rn ?><?php echo $row['ans']; ?><?php echo $i+1 ?> {
		...
		<div id="squestion<?php $rn = rand(); ?>1<?php echo ($i+1) ?>">
			<table>
				<tr>
					<td>A.</td>
		...
		<div id="squestion<?php $rn = rand(); ?>2<?php echo ($i+1) ?>">
		...
		<div id="squestion<?php $rn = rand(); ?>3<?php echo ($i+1) ?>">
		<table>
			<tr>
				<td>C.</td>
		...
		<div id="squestion<?php $rn = rand(); ?>4<?php echo ($i+1) ?>">
			<table>
			<tr>
				<td>D.</td>
