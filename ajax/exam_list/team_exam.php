...
session_start();
 $user = user_detail("user_name");
	$sql = "SELECT * FROM team_score WHERE user='$user' AND date LIKE '%$search%' GROUP BY date ORDER BY id DESC LIMIT $crt,$limit";
...
{
	$.ajax({
	url: '<?php get_domain("/ajax/") ?>exam_list/team_exam_question.php',
	type: 'GET',
	data: "id="+id,
})
.done(function(dta) {
...
<td style="font-family: arial;"><?php echo number_format($row['score'],2); ?></td>
<td style="font-family: arial;"><?php echo get_team($row['team']) ?></td>
<td style="font-family: arial;"><?php echo get_chal($row['chal']) ?></td>
<!-- <td class="rtr25" style="font-family: arial;"><?php echo number_format($penalty,3); ?> Min</td> -->
<td style="font-family: arial;"><?php echo date("h:ia, d M Y",strtotime($row['date'])) ?></td>
...
<?php 
$sql = "SELECT * FROM team_score WHERE user='$user' AND date LIKE '%$search%' GROUP BY date ORDER BY id DESC";
