...
$user = user_detail("user_name");
	$sql = "SELECT * FROM custom_score WHERE user='$user' AND date LIKE '%$search%' GROUP BY date ORDER BY id DESC LIMIT $crt,$limit";
...
{
	$.ajax({
	url: '<?php get_domain("/ajax/") ?>exam_list/question_custom.php',
	type: 'GET',
	data: "id="+id,
})
.done(function(dta) {
...
<td style="font-family: arial;"><?php echo $row['chapter'] ?></td>
<td style="font-family: arial;"><?php echo $row['subject'] ?></td>
...
<?php 
$sql = "SELECT * FROM custom_score WHERE user='$user' AND date LIKE '%$search%' GROUP BY date ORDER BY id DESC";
