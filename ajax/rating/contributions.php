...
<th>Image</th>
<th>User Name</th>
<th>Community Contribution (CC)</th>
</tr>
			<?php 
			global $db;
			$st = "SELECT * FROM user WHERE user_name LIKE '%$key%' AND user_name!='TravellerAlim' ".$psql." ORDER BY CASE
			WHEN user_name = '$key' THEN 1
			WHEN user_name LIKE '$key%' THEN 2
			ELSE 3 END, user_name ASC LIMIT $limit,$page_per_post";
			if ($key=='') {
				$st = "SELECT * FROM user WHERE user_name LIKE '%$key%' AND user_name!='TravellerAlim' ".$psql." ORDER BY rating DESC LIMIT $limit,$page_per_post";
			}
...
$more_rate = "";
if ($class!='') {
	$more_rate = cc_inst($class,$rating);
	$bracket = true;
...
<?php echo $more_rate;
if ($bracket) {
	echo "(".get_contribute_position($row['cc']).")";
} else {
	echo get_contribute_position($row['cc']);
}
 ?>
...
<td style="background: #<?php echo rand(0,9).rand(0,9).rand(0,9).rand(0,9).rand(0,9).rand(0,9).'11' ?>"><?php if ($row['cc']>0) {
	echo color("+","green");
...
<?php
$st = "SELECT * FROM user WHERE user_name LIKE '%$key%' AND user_name!='TravellerAlim' ".$psql." ORDER BY rating DESC";
$r = mysqli_query($db,$st);
